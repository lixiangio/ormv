...
class Parser {
  ...
  object(express, data, key) {
    ...
    // express为对象结构
      ...
      for (const sKey in express) {
        const { error, data: subData } = this.recursion(
          express[sKey],
          data[sKey],
          sKey
        );
  ...
  expression(express, data, key) {
    ...
    const type = types.get(express.type);
    // type为内置数据类型
    if (type) {
 ...
 */
function validator(express, data, mode) {
...
function typea(express) {
  return {
     ...
     */
    verify(data) {
      return validator(express, data, undefined);
    },
    /**
     * 严格模式
     * 禁止所有空值，有值验证，无值报错
     */
    strictVerify(data) {
      return validator(express, data, "strict");
    },
    /**
     * 宽松模式
     ...
     */
    looseVerify(data) {
      return validator(express, data, "loose");
    }
  };
}
