'use strict';

const filter = require('../filter.js');

const { sqlKey, sqlValue } = filter;

const op = Symbol.for('op');

/**
 * 逻辑选项转sql
 * @param {Array} parameter 每个子节点均为and集合，多个子节点之间or集合
 */
function Logic(parameter) {

   parameter = parameter.filter(item => item);

   if (parameter.length === 0) return;

   const orArray = [];

   for (const andOptions of parameter) {

      const andArray = [];

      for (const name in andOptions) {

         const value = andOptions[name];

         if (value instanceof Object) {

            const opValue = value[op];

            // 操作符
            if (opValue) {
               andArray.push(`${sqlKey(name)} ${opValue}`);
            }

            // 对象
            else {
               andArray.push(`${sqlKey(name)} = ${sqlValue(JSON.stringify(value))}`);
            }

         } else {

            andArray.push(`${sqlKey(name)} = ${sqlValue(value)}`);

         }

      }

      const sql = andArray.join(" AND ");

      orArray.push(`(${sql})`);

   }

   if (orArray.length === 1) {

      return orArray[0];

   } else {

      const sql = orArray.join(" OR ");

      return `(${sql})`;

   }

}

/**
 * select函数链
 */
module.exports = class {
   constructor() {

      this.logic = [];

   }
   /**
    * 参数与and相同
    * @param {*} parameter 
    */
   where(...parameter) {

      const sql = Logic(parameter);

      if (sql) {

         this.logic.push(` WHERE ${sql}`);

      } else {

         this.logic.push(` WHERE TRUE`);

      }

      return this;

   }
   and(...parameter) {

      const sql = Logic(parameter);

      if (sql) {

         this.logic.push(` AND ${sql}`);

      }

      return this;

   }
   or(...parameter) {

      const sql = Logic(parameter);

      if (sql) {
         this.logic.push(` OR ${sql}`);
      }

      return this;

   }
   /**
    * Promise对象then()方法
    * @param {Function} resolve 
    * @param {Function} reject 
    */
   then(resolve, reject) {
      
      return this.promise().then(resolve, reject);

   }
   catch(reject) {

      return this.promise().catch(reject);

   }
}