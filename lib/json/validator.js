...
class Parser {
  ...
  verify(data) {
    ...
    this.origin = data;
    return this.recursion(this.express, data, "");
  ...
  expression(express, data) {
    ...
    if (!typeFunc) {
      return { error: `不支持"${express.type}"类型` };
    }
 ...
 * @param {*} express 验证表达式
 */
function validator(express) {

  const parser = new Parser(express);
