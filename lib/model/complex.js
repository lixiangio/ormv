...
class Model {
   ...
   constructor(client, options) {
      ...
      const { fieldIndex, fields } = this;
      const mergeFields = options.fields;

      if (mergeFields === undefined) {
         throw new Error(`复合模型${modelNames}缺少fields字段配置项`);
      }

      for (const pathName of mergeFields) {
         ...
         if (typeof pathName === 'string') {
            ...
            if (fieldInfo) {
               ...
               if (fieldIndex[fieldName]) {
                  ...
                  fieldIndex[fieldName] = `"${modelName}"."${fieldName}"`;
            ...
            const fieldInfo = allFields[name];

            if (fieldInfo) {
               if (fieldIndex[alias]) {
                  throw new Error(`${modelNames}复合模型中${alias}字段存在命名冲突`);
               } else {
                  fieldIndex[alias] = pathName[op];
                  fields[alias] = fieldInfo.options;
               }
            } else {
               throw new Error(`${modelNames}复合模型中未找到${pathName}字段`);
            }
      ...
      for (const name in fieldIndex) {
         select.push(fieldIndex[name]);
      }
