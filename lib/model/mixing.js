...
class Model {
   ...
   constructor(client, options) {
      ...
      if (options.innerJoin) {
         ...
         this.joinModule = options.innerJoin;
      } else if (options.leftJoin) {
         this.joinType = "LEFT JOIN";
         this.joinModule = options.leftJoin;
      } else if (options.rightJoin) {
         this.joinType = "RIGHT JOIN";
         this.joinModule = options.rightJoin;
      } else {
      ...
      for (const module of [this.fromModule, this.joinModule]) {
      ...
      this.name = `"${this.fromModule.name}_${this.joinModule.name}"`;
   ...
   _findCb() {
      ...
      return {
         ...
         "from": {
            key: "FROM",
            value: `"${this.schemaName}"."${this.fromModule.name}"`
         },
         "join": {
            key: this.joinType,
            value: `"${this.schemaName}"."${this.joinModule.name}"`
         },
         on: { ...this.on }
      }
